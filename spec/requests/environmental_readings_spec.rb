...
RSpec.describe "EnvironmentalReadings API" do
  ...
  describe "GET /environmental_sensors/:sensor_id/readings" do
    context "with HTML format" do
      it "returns success" do
        get environmental_sensor_readings_path(sensor)
    ...
    context "with JSON format" do
      it "returns all readings for sensor" do
        get environmental_sensor_readings_path(sensor, format: :json)
      ...
      it "orders readings by recorded_at descending" do
        ...
        json = response.parsed_body
        expect(json.first["id"]).to eq(reading2.id)
        expect(json.last["id"]).to eq(reading1.id)
      end
    end
  end
  ...
  describe "POST /environmental_sensors/:sensor_id/readings" do
    ...
    context "with valid parameters" do
      it "creates a new reading" do
        expect do
          post environmental_sensor_readings_path(sensor), params: { environmental_reading: valid_attributes }
      ...
      it "creates alert if threshold exceeded" do
        ...
        expect do
          post environmental_sensor_readings_path(sensor), params: { environmental_reading: exceeding_attributes }
    ...
    context "with invalid parameters" do
      ...
      it "does not create a reading" do
        expect do
          post environmental_sensor_readings_path(sensor), params: { environmental_reading: invalid_attributes }
  ...
  describe "DELETE /environmental_sensors/:sensor_id/readings/:id" do
    it "destroys the reading" do
      expect do
        delete environmental_sensor_reading_path(sensor, older_reading)
      end.to change(sensor.environmental_readings, :count).by(-1)
    end
  end
end
